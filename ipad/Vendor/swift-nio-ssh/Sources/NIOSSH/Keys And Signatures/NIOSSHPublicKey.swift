...
extension NIOSSHPublicKey {
    ...
    internal func isValidSignature<DigestBytes: Digest>(_ signature: NIOSSHSignature, for digest: DigestBytes) -> Bool {
        ...
        case (.rsa(let key), .rsa(let sig, _)):
            // The digest carries the hash algorithm: the RSA verifier derives the
            // PKCS#1 v1.5 DigestInfo OID from the digest type.
            return key.isValidSignature(
                _RSA.Signing.RSASignature(rawRepresentation: sig.dataRepresentation),
                for: digest,
    ...
    internal func isValidSignature(_ signature: NIOSSHSignature, for bytes: ByteBuffer) -> Bool {
        ...
        case (.ecdsaP521(let key), .ecdsaP521(let sig)):
            return key.isValidSignature(sig, for: bytes.readableBytesView)
        case (.rsa(let key), .rsa(let sig, let algorithm)):
            return key.isValidRSASignature(sig, algorithm: algorithm, over: bytes.readableBytesView)
        case (.certified(let key), _):
    ...
    internal func isValidSignature(_ signature: NIOSSHSignature, for payload: UserAuthSignablePayload) -> Bool {
        ...
        case (.ecdsaP521(let key), .ecdsaP521(let sig)):
            return key.isValidSignature(sig, for: payload.bytes.readableBytesView)
        case (.rsa(let key), .rsa(let sig, let algorithm)):
            return key.isValidRSASignature(sig, algorithm: algorithm, over: payload.bytes.readableBytesView)
        case (.certified(let key), _):
...
extension _RSA.Signing.PublicKey {
    /// Verifies an RSA signature over the given message bytes, hashing them with the
    /// hash function implied by the signature algorithm (RFC 8332).
    fileprivate func isValidRSASignature<Bytes: DataProtocol>(
        _ signature: NIOSSHSignature.BackingSignature.RawBytes,
        algorithm: NIOSSHSignature.BackingSignature.RSAAlgorithm,
        over bytes: Bytes
    ) -> Bool {
        let rsaSignature = _RSA.Signing.RSASignature(rawRepresentation: signature.dataRepresentation)

        switch algorithm {
        case .sha1:
            return self.isValidSignature(
                rsaSignature, for: Insecure.SHA1.hash(data: bytes), padding: .insecurePKCS1v1_5
            )
        case .sha256:
            return self.isValidSignature(
                rsaSignature, for: SHA256.hash(data: bytes), padding: .insecurePKCS1v1_5
            )
        case .sha512:
            return self.isValidSignature(
                rsaSignature, for: SHA512.hash(data: bytes), padding: .insecurePKCS1v1_5
            )
        }
    }
}
...
extension NIOSSHPublicKey {
    ...
    /// The public key algorithm name to use in user authentication messages.
    ///
    /// For RSA keys this is the signature algorithm name rather than the key type name
    /// (RFC 8332 §3): the key blob remains "ssh-rsa", but the algorithm name field must
    /// agree with the signature, which is always produced as "rsa-sha2-256".
    internal var userAuthAlgorithmName: String.UTF8View {
        switch self.backingKey {
        case .rsa:
...
extension ByteBuffer {
    ...
    private mutating func writeRSAPublicKey(baseKey: _RSA.Signing.PublicKey) -> Int {
        // For RSA, the key format is `mpint e` followed by `mpint n` (RFC 4253 §6.6).
        // getKeyPrimitives cannot fail here: the underlying BoringSSL implementation is
        // non-throwing for any key that was successfully constructed.
    ...
    private mutating func readRSAPublicKey() throws -> NIOSSHPublicKey? {
        // For RSA, the key format is `mpint e` followed by `mpint n` (RFC 4253 §6.6).
        // We read them as SSH strings: any leading zero byte added to keep the mpint
        // positive is harmless, as BoringSSL normalises the big-endian representation.
        guard let eBytes = self.readSSHString(), let nBytes = self.readSSHString() else {
            return nil
        }

        let key = try _RSA.Signing.PublicKey(
            n: Data(nBytes.readableBytesView),
            e: Data(eBytes.readableBytesView)
        )

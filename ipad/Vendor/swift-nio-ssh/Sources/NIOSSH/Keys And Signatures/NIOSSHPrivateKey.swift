...
extension NIOSSHPrivateKey {
    func sign<DigestBytes: Digest>(digest: DigestBytes) throws -> NIOSSHSignature {
        ...
        case .rsa(let key):
            // RSA signatures in SSH use PKCS#1 v1.5 padding over the exchange hash digest.
            // We always advertise the signature as rsa-sha2-256 (RFC 8332).
            let signature = try key.signature(for: digest, padding: .insecurePKCS1v1_5)
            return NIOSSHSignature(backingSignature: .rsa(.data(signature.rawRepresentation), algorithm: .sha256))
    ...
    func sign(_ payload: UserAuthSignablePayload) throws -> NIOSSHSignature {
        ...
        case .ecdsaP521(let key):
            ...
            return NIOSSHSignature(backingSignature: .ecdsaP521(signature))
        case .rsa(let key):
            // RFC 8332: RSA clients sign with rsa-sha2-256: SHA-256 over the payload, PKCS#1 v1.5 padding.
            // The key blob itself remains "ssh-rsa".

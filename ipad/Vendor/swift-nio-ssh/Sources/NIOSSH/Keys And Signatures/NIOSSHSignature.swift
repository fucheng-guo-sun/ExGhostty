...
extension NIOSSHSignature {
    ...
    internal enum BackingSignature: Sendable {
        ...
        case rsa(RawBytes, algorithm: RSAAlgorithm)
        ...
        /// The hash algorithm used with an RSA signature (RFC 8332).
        internal enum RSAAlgorithm: Sendable, Hashable {
            case sha1
            case sha256
            case sha512
        }
    ...
    fileprivate static let rsaSha512SignaturePrefix = "rsa-sha2-512".utf8

    /// The prefix of an RSA signature using SHA-1.
    fileprivate static let rsaSha1SignaturePrefix = "ssh-rsa".utf8
}
...
extension NIOSSHSignature.BackingSignature.RawBytes {
    /// The signature bytes as a `Data` value.
...
extension NIOSSHSignature.BackingSignature: Equatable {
    static func == (lhs: NIOSSHSignature.BackingSignature, rhs: NIOSSHSignature.BackingSignature) -> Bool {
        ...
        case (.ecdsaP521(let lhs), .ecdsaP521(let rhs)):
            return lhs.rawRepresentation == rhs.rawRepresentation
        case (.rsa(let lhsBytes, let lhsAlgorithm), .rsa(let rhsBytes, let rhsAlgorithm)):
            return lhsBytes == rhsBytes && lhsAlgorithm == rhsAlgorithm
        case (.ed25519, _),
            (.ecdsaP256, _),
            (.ecdsaP384, _),
            (.ecdsaP521, _),
...
extension NIOSSHSignature.BackingSignature: Hashable {
    func hash(into hasher: inout Hasher) {
        ...
        case .ecdsaP521(let sig):
            hasher.combine(3)
            hasher.combine(sig.rawRepresentation)
        case .rsa(let bytes, let algorithm):
            hasher.combine(4)
            hasher.combine(bytes)
...
extension ByteBuffer {
    ...
    mutating func writeSSHSignature(_ sig: NIOSSHSignature) -> Int {
        ...
        case .rsa(let sig, let algorithm):
            return self.writeRSASignature(signatureBytes: sig, algorithm: algorithm)
        }
    }
    ...
    private mutating func writeRSASignature(
        ...
        // The RSA signature format is easy: the signature algorithm prefix, followed by
        // the raw signature bytes. RSA signatures are fixed-width raw bytes, no further
        // structure is needed.
        let prefix: String.UTF8View
        switch algorithm {
        case .sha1:
            prefix = NIOSSHSignature.rsaSha1SignaturePrefix
        case .sha256:
            prefix = NIOSSHSignature.rsaSha256SignaturePrefix
        case .sha512:
            prefix = NIOSSHSignature.rsaSha512SignaturePrefix
        }
    ...
    mutating func readSSHSignature() throws -> NIOSSHSignature? {
        try self.rewindOnNilOrError { buffer in
            ...
            if bytesView.elementsEqual(NIOSSHSignature.ed25519SignaturePrefix) {
                ...
                return try buffer.readRSASignature(algorithm: .sha256)
            } else if bytesView.elementsEqual(NIOSSHSignature.rsaSha512SignaturePrefix) {
                return try buffer.readRSASignature(algorithm: .sha512)
            } else if bytesView.elementsEqual(NIOSSHSignature.rsaSha1SignaturePrefix) {
                return try buffer.readRSASignature(algorithm: .sha1)
            } else {
    ...
    /// Not safe to call from arbitrary code as this does not return the reader index on failure: it relies on the caller performing
    /// the rewind.
    private mutating func readRSASignature(algorithm: NIOSSHSignature.BackingSignature.RSAAlgorithm) throws
        -> NIOSSHSignature?
    {
        // For RSA the signature is just the raw signature bytes encoded as a String.
        guard let sigBytes = self.readSSHString() else {
            return nil
        }

        return NIOSSHSignature(backingSignature: .rsa(.byteBuffer(sigBytes), algorithm: algorithm))
    }
